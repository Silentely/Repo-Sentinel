...
| 时间戳 (UTC) | 变更摘要 |
|---|---|
| 2026-09-23T00:00:00Z | 健壮性与可观测性收敛：①对账工作项读写失败不再静默吞掉（写入失败留痕 `reconcile_upsert_failed` 并计软失败、读取失败区分 `store.ErrNotFound` 且不触发 enrich、失败时不推进游标下轮重拉补齐，事件指纹幂等不重复通知）；②GitHub 运行时配置解析与信封解密失败不再静默降级（库内 JSON 损坏、主密钥轮换后旧信封解不开均返回指明字段的错误，此前表现为入站 webhook 全被 503 而管理台仍显示已配置）；③外部轮询归档收口失败补 `repo_state_update_failed` Warn，`PollOne` 客户端惰性初始化改只读回退消除并发写字段；④star 同步用户名按 GitHub 字符集收敛写入侧（400 `validation_failed` 指明 `field=username`）与消费侧（跳过本轮 + Warn 留痕 + 路径 `url.PathEscape` 兜底）边界；⑤审计写入四处（2FA 启用/停用、仓库删除、CLI 应急重置）改统一经 `server.appendAudit`，失败 Warn 留痕 `audit_append_failed`；⑥`/api/v1/stats/actions-insights` 分析窗口按 300 条样本取（逐页拉取、末页不足一页提前收尾、超上限截断）；⑦AI 重试耗尽的错误文案追加 `attempts=N`（保留错误码与 Unwrap 链），单次与连续失败可区分；⑧PR 合并置位改依据解析出的 `repo.ID`，不再解引用 `*res.Event.RepositoryID`（事件行缺仓库关联时不再 panic）；⑨前端配置表单回填补「仅首次」守卫（设置页/GitHub 页/Star Release 页，mutation invalidate 后 refetch 不再覆盖未保存编辑）、AIReviewCard 与 Star Release 同步轮询卸载即停、复制定时器卸载即清、审查卡片 memo、仪表盘与设置页派生数组 memo 修正无效缓存、审查结论三类清单抽 `ReviewRiskList` 组件、Webhook 页码同步 URL 且非法页码回退第 1 页、2FA 状态读取失败不再谎报未开启、载荷查询失败不再误报记录不存在、Inspector 接入统一模态层、主题与 2FA 剩余次数收敛函数取代强转；⑩接入 oxlint 静态检查（typescript 7.0 超出 typescript-eslint 支持范围，类型正确性仍由 `tsc --noEmit` 保证）并据结果清理死导入；各类改动均补回归测试并先还原实现验证可捕获缺陷 |
| 2026-09-23T00:00:00Z | 版本 v0.6.1（补丁）：①`notification_outbox` 冗余列 `repository_full_name` 与 `(status, repository_full_name)` 索引，unstar 取消未投递 Release 通知改单条批量 UPDATE；②仪表盘同表分组聚合、外部仓轮询有界并发、对账 PR 并发扇出、发件箱渠道懒加载、PR 审查在途前缀拒绝与结果单条直查；③AI 出站与配置探测禁用 30x 重定向跟随、配置与拨号双层级联拦截云元数据地址防 SSRF、TLS/`X-Forwarded-Proto` 自动追加会话与 CSRF Cookie Secure 属性、仓库硬删除与 2FA 操作补审计落库、`/metrics` 未授权日志按 IP 采样；④启用 2FA 会话校验前置落库；⑤修复已取消星标仓库仍被轮询推送（`rel="next"` 翻页依据 + `last_star_sync_at` 落定刷新）、私有仓手动审查安装解析、`PollAll` 取消误报成功、保留策略清理在途通知、webhook 列表 queryKey 标量化 |
| 2026-09-20T00:00:00Z | 管理台热点查询收敛：①`notification_outbox` 新增冗余列 `repository_full_name` 与 `(status, repository_full_name)` 索引，取消 unstar 仓的未投递 Release 通知改单条批量 UPDATE（SQLite/PostgreSQL 双轨迁移）；②仪表盘统计同表维度合并分组聚合（work_items 按 kind、repositories 按 sync_status），整页刷新 SQL 往返减少；③活跃/归档仓 ID 集合缓存随附 `id→full_name` 映射，资源列表页解析仓库名复用同一份扫描；④外部仓轮询改有界并发（5 worker，限流即停整轮、逐仓失败不波及其余）；⑤对账 PR 补数据改并发拉取；⑥发件箱渠道映射改懒加载、`/metrics` 未授权访问日志按 IP 采样、2FA 会话校验前置于落库、AI Base URL 校验仅拦截云元数据域名；⑦列表查询保留「COUNT + 取页」两次往返与活跃仓 IN 列表：实测 SQLite 2 万行事件 1.45ms、2000 活跃仓 2.75ms，绑定参数上限 32766，均远超目标规模且单查询改写需复制 7 处过滤条件（schema 漂移风险），取证结论记入 `internal/store/CLAUDE.md` FAQ；⑧补 PR 审查安装 ID 解析与审查结果幂等判定的同包回归测试 |
