...
| AI 摘要 / 告警分诊 | 已交付 | 可选（默认关闭）；OpenAI 兼容端点，可接本地模型；管理台可编辑（env 优先、密钥加密存库、热生效）；AI 不可用时自动降级为模板/原文 |
| Outbox 重试 / 死信 | 已交付 | |
...
7. AI 功能默认关闭：未配置 `REPOSENTINEL_AI_API_KEY` 时不发起任何外部请求；AI 输出经 HTML 转义后嵌入通知，失败自动降级不影响投递。  
