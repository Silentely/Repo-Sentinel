...
| 2026-09-25T08:00:00Z | 优雅关停日志降噪：`runScheduledTask` 与 `ExternalPoller.pollRepo` 识别父级 context 取消（`ctx.Err() != nil`），关停期间中止的任务记录为 DEBUG（或直接返回），不再输出误报的 ERROR 错误日志 |
| 2026-09-25T07:30:00Z | 外部轮询与 Release 事件健壮性优化：①`ExternalPoller` 在 work item upsert 失败时设置 `softFailed` 与 `external_partial` 错误码，跳过推进 issues 游标防止数据静默丢失；②`collapseArchived` 统一归拢 installation 归档仓库停用；③`StarredReleasePoller.createReleaseEvent` 对 tag_name、name、actor、url 严格修剪空白字符并在无标题时安全回退 `Release <id>` |
