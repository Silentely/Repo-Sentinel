...
| 时间戳 (UTC) | 变更摘要 |
|---|---|
| 2026-09-25T13:35:00Z | 删除 `outboxErrorHint` 的 `missing_target` 不可达分支：后端无任何路径产出该错误码（Telegram 目标为空产出 `telegram_not_configured`，Webhook 目标非法产出 `invalid_webhook_url`），提示永不渲染且其测试断言构成虚假覆盖 |
| 2026-09-25T11:25:00Z | 优化 Star Release 列表项重渲染性能：抽取 TrackerRowItem 结合 React.memo 保持 handleToggle 引用稳定，消除表单输入与状态轮询时全量追踪行的冗余重渲染 |
